...
const SPAM_KEYWORDS: string[] = [
  'seo services', 'seo optimization', 'search engine optimization',
  'rank your website', 'first page of google', 'google ranking',
  'backlinks', 'link building', 'guest post', 'guest posting',
  'sponsored post', 'buy traffic', 'increase traffic', 'website traffic',
  'organic traffic', 'web development services', 'website redesign',
  'mobile app development', 'offshore development', 'hire developers',
  'dedicated developers', 'email marketing', 'email list',
  'lead generation', 'b2b leads', 'qualified leads',
  'social media marketing', 'digital marketing agency', 'marketing services',
  'i was impressed by your website', 'i came across your website',
  'i noticed your website', 'we can help you', 'we are offering',
  'special offer', 'limited time offer', 'guaranteed results',
  'kindly reply', 'kindly revert', 'do the needful',
  'awaiting your response', 'looking forward to your positive response',
]
...
const KEYBOARD_PATTERNS = [
  'qwert', 'asdf', 'zxcv', 'qwerty', 'asdfg', 'zxcvb',
...
const COMMON_WORDS = new Set([
  ...
  'hello', 'hi', 'thanks', 'please', 'help', 'need', 'question',
  'contact', 'message', 'email', 'website', 'project', 'interested',
  'opportunity', 'work', 'working', 'love', 'great', 'am', 'is',
  'are', 'was', 'been', 'being', 'how', 'want', 'wanted', 'should',
])

const DISPOSABLE_EMAIL_DOMAINS = [
  'tempmail', 'temp-mail', 'guerrillamail', 'mailinator',
  '10minutemail', 'throwaway', 'fakeinbox', 'trashmail',
]
...
function calculateGibberishScore(text: string): number {
  ...
  if (vowelRatio < 0.15 || vowelRatio > 0.6) score += 25
  else if (vowelRatio < 0.25 || vowelRatio > 0.5) score += 10

  const clusters = findConsonantClusters(text)
  score += Math.min(clusters.length * 15, 30)

  if (hasKeyboardPatterns(text)) score += 25

  const wordRatio = getCommonWordRatio(text)
  if (wordRatio < 0.2) score += 20
  else if (wordRatio < 0.4) score += 10
...
export function checkForSpam(data: ContactData): SpamCheckResult {
  ...
  if (data.website && data.website.trim() !== '') {
    return { isSpam: true, reasons: ['Honeypot field was filled (bot detected)'], score: 100 }
  }
  ...
  if (messageGibberishScore >= 50) {
    reasons.push(`Message appears to be gibberish (score: ${messageGibberishScore})`)
    score += 25
  }

  const combinedText = `${data.name} ${data.message}`.toLowerCase()
  const matchedKeywords = SPAM_KEYWORDS.filter((keyword) => combinedText.includes(keyword.toLowerCase()))
  ...
  return { isSpam: score >= 40, reasons, score: Math.min(score, 100) }
}
